    ...
    for element, attribute in elements. items():
        ...
        for idx, val in enumerate(images):
            ...
            if url:
                ...
                contents = contents.replace(to_unicode(url), to_unicode(absolute_new_filename))
...
def fetch_file(base, url, root, extension, article, user, handle_images=False):
    ...
    with codecs.open(os.path.join(path, filename), 'wb') as f:
        ...
        if handle_images:
            resp = to_unicode(fetch_images_and_rewrite_xml_paths(base, root, resp, article, user))

        f.write(to_bytes(resp))
...
def save_file(base, contents, root, extension, article, user, handle_images=False):
    ...
    with codecs.open(os.path.join(path, filename), 'wb') as f:
        ...
        if handle_images:
            contents = to_unicode(fetch_images_and_rewrite_xml_paths(base, root, contents, article, user))
...
def add_file(file_mime, extension, description, owner, filename, article, galley=True, thumbnail=False):
    ...
    new_file.add_root(instance=new_file)
...
def set_article_attributions(authors, emails, institutions, mismatch, article):
    ...
    for idx, val in enumerate(authors):
        ...
        if fetch_emails:
            ...
            email = logic.generate_password(16)
